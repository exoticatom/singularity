...
**Project:** singularity | **Firmware:** v1.2.0 | **Last updated:** 2026-09-19
...
| **MCP4728 DAC** *(firmware ready, unwired)* | 0x60 quad DAC on +5 V → 0-5 V VOUT A → proportional valve; BSS138 bi-dir level shifter bridges the 3.3 V I2C bus | [expansion_boards.md](../hardware/expansion_boards.md#proportional-valve-wiring--mcp4728-on-5v-with-i2c-level-shifter) |
...
- The **MCP4728 DAC + BSS138 level shifter** block is drawn dashed because the board is **not yet physically wired**. It *is* integrated in firmware (`mcp4728` hub @ 0x60 → `output` channel A → `number.singularity_proportional_valve`, a 0–100 % slider on the main dashboard) — see [expansion_boards.md](../hardware/expansion_boards.md). It is the *only* device on +5 V (for a direct 0-5 V valve signal); ADS1115 and MCP23017 stay on the 3.3 V side of the shifter. Its SDA/SCL join the shared I2C bus (GPIO21/47), shown via net-labels rather than long wires.
